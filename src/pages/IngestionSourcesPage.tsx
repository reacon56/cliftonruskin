...
export default function IngestionSourcesPage() {
  ...
  const { data: runs = [] } = useQuery({
    queryKey: ["ingestion-runs", selectedSourceId],
  ...
  const triggerRun = useMutation({
    ...
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ["ingestion-runs", selectedSourceId] });
      qc.invalidateQueries({ queryKey: ["data-sources"] });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground flex items-center gap-2">
            <Server className="h-6 w-6 text-primary" /> Ingestion Sources
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Configure and monitor data ingestion sources for jurisdiction intelligence</p>
        </div>
        {isManager && (
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No ingestion sources configured</TableCell></TableRow>
              ) : sources.map((s) => (
                <TableRow
                  key={s.id}
                  className={`cursor-pointer ${!s.is_active ? "opacity-50" : ""} ${selectedSourceId === s.id ? "bg-muted/50" : ""}`}
            ...
            <>
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm">{selectedSource.name}</CardTitle>
                    {isManager && (
                      <Button size="sm" variant="outline" onClick={() => triggerRun.mutate()} disabled={triggerRun.isPending}>
                        <Play className="h-3.5 w-3.5 mr-1" /> {triggerRun.isPending ? "Running…" : "Run Now"}
                      </Button>
                    )}
                  </div>
                </CardHeader>
                <CardContent className="space-y-2">
                  {selectedSource.base_url && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Base URL</p>
                      <p className="text-xs text-foreground break-all">{selectedSource.base_url}</p>
                    </div>
                  )}
                  {selectedSource.urls?.length > 0 && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">URLs ({selectedSource.urls.length})</p>
                      <div className="space-y-0.5 mt-1">
                        {selectedSource.urls.slice(0, 3).map((u, i) => (
                          <p key={i} className="text-[11px] text-muted-foreground truncate">{u}</p>
                        ))}
                        {selectedSource.urls.length > 3 && (
                          <p className="text-[10px] text-muted-foreground/60">+{selectedSource.urls.length - 3} more</p>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <h3 className="font-display text-sm font-semibold text-foreground">Run History</h3>
              {runs.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-4">No runs recorded</p>
              ) : (
                    ...
                    <Card key={r.id} className="p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {format(new Date(r.started_at), "dd MMM yyyy HH:mm")}
                        </span>
                      </div>
                      <div className="flex gap-4 text-[11px] text-muted-foreground">
                        <span>Processed: {r.records_processed}</span>
                        <span>Changed: {r.records_changed}</span>
                        {r.finished_at && (
                          <span>Duration: {Math.round((new Date(r.finished_at).getTime() - new Date(r.started_at).getTime()) / 1000)}s</span>
                        )}
                      </div>
                    </Card>
                  ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editId ? "Edit Ingestion Source" : "Add Ingestion Source"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={form.expected_format} onValueChange={(v) => setForm({ ...form, expected_format: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {EXPECTED_FORMATS.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div>
              ...
              <Select value={form.refresh_cadence} onValueChange={(v) => setForm({ ...form, refresh_cadence: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {REFRESH_CADENCES.map((c) => <SelectItem key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} placeholder="What this source provides…" />
            </div>
