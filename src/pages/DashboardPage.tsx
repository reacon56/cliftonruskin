...
export default function DashboardPage() {
  ...
  const loadDashboard = async () => {
    ...
    const [
      ...
      supabase.from("cases").select("id", { count: "exact", head: true }).eq("org_id", orgId).not("status", "in", '("delivered","closed","cancelled")'),
      supabase.from("cases").select("id", { count: "exact", head: true }).eq("org_id", orgId).in("status", ["delivered", "closed"]).gte("created_at", monthStart),
      ...
      supabase.from("cases").select("id", { count: "exact", head: true }).eq("org_id", orgId).in("status", ["submitted", "quoted"]),
