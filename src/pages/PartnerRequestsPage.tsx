...
import { MapPin, Calendar, ChevronRight, Send } from "lucide-react";
import {
...
export default function PartnerRequestsPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("id, title, country, deadline, status, created_at, partner_id")
  ...
  return (
    <div>
      <h1 className="fvc-heading-1 text-foreground mb-1">Partner Requests</h1>
      <div className="fvc-gold-rule mt-3 mb-2" />
      <p className="text-sm text-muted-foreground mb-8">
        In-country tasks you have sent to the partner network
      </p>

      {tasks.length === 0 ? (
        <div className="fvc-card text-center py-12">
          <Send size={24} className="mx-auto text-muted-foreground/40 mb-3" />
          <p className="text-sm text-muted-foreground">No partner requests created yet.</p>
        </div>
      ) : (
